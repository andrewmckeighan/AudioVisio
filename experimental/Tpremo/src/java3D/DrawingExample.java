 ...
 * I've just copied this code as a proof of interaction with the user, and 3d display.
 * I've added my own comments to better understand how the library works.
 * This Library seems fairly lightweight, and it might be hard to render complicated shapes (a player), and isn't build for game development.
 * It would be a considerable amount of work to use this as our renderer as all physics would be done by hand.
 * We should decide if we want to use a game/physics engine, or work on our own.
 */
...
public class DrawingExample extends Applet implements MouseListener, MouseMotionListener {
	...
	private void startDrawing() {
		...
		canvas = new Canvas3D(config); //display of the universe (the camera)
		universe = new SimpleUniverse(canvas); //virtual area where 'objects' exist.
		add("Center", canvas);
		positionViewer();
		getScene();
	...
	public void getScene() {
		...
		frontShape.setAppearance(ap);
			...
			//set the individual sides of the box to be different colors.
		...
		box.getShape(Box.BACK).setAppearance(getAppearance(new Color3f(Color.yellow))); ;
			...
			//set the front side to be a image dependent on the user's drawing.
	...
	/**
	 * adds the drawing image onto the face of the box.
	 * @param image drawing from the user
	 * @param shape face of the box
	 */
	...
	/**
	 * Generates the original front face of the box
	 * @param i
	 * @param width
	 * @param height
	 * @return
